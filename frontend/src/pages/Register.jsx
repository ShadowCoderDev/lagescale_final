  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userData) {
        ...
        setError(
          "ثبت‌نام موفق بود اما اطلاعات کاربر دریافت نشد. لطفاً وارد شوید.",
        );
      }
    } catch (err) {
      ...
      if (errorData) {
        if (typeof errorData === "object" && !errorData.message) {
          ...
          setError(
            errorMessages ||
              "اعتبارسنجی ناموفق بود. لطفاً ورودی‌های خود را بررسی کنید.",
          );
        } else {
          setError(
            errorData.error ||
        ...
        setError(
          `ثبت‌نام ناموفق بود (وضعیت: ${err.response?.status || "نامشخص"}). لطفاً دوباره تلاش کنید.`,
        );
      }
    } finally {
      setLoading(false);
